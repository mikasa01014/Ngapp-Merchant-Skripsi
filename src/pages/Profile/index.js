import React, { useState, useEffect } from 'react'
import { StyleSheet, Text, View, Image, TouchableOpacity, Alert } from 'react-native'
import { Button, AuthContext } from '../../component'
import { colors as constantColors } from '../../constant/colors'
import { User } from '../../asset'

import Icon from 'react-native-vector-icons/MaterialCommunityIcons'
import axios from 'axios'
import { ScrollView } from 'react-native-gesture-handler'
import AsyncStorage from '@react-native-async-storage/async-storage'

const Profile = ({ navigation, route }) => {
    var userProfilePicture = ''

    const [user, setUser] = useState([])

    const getMerchantData = () => {
        axios.defaults.headers.common['Authorization'] = `Bearer ${global.token}`
        axios.get('https://ngapp-backend.herokuapp.com/menu/merchant/profile')
            .then((res) => {
                console.log(res.data.merchant)
                setUser(res.data.merchant)
                //console.log(user)

                //AsyncStorage.setItem('user', JSON.stringify(merchant))    
            }).catch (e => {

            })
    }

    
    useEffect(() => {
       getMerchantData();
    }, [])


    const deleteToken = async () => {
        try {
            await AsyncStorage.removeItem('token');
    
        } catch( error ) {
            console.log("error")
        }
    }

    const signOut = () => {
        Alert.alert(
            'Logout',
            'Do you want to Log Out ?',
            [
                {
                    text : 'Cancel',
                    style : 'cancel',
                },
                {
                    text: 'Yes', 
                    onPress: () => {
                        deleteToken()
                        navigation.replace('WelcomeAuth')
                    }
                }
            ],
            {cancelable: false}
        )
    }


    

    const options = [
        {
            title: 'View Menu',
            icon: 'food',
            action: () => { navigation.navigate('Menu') }
        },
        {
            title: 'View Orders',
            icon: 'archive-outline',
            action: () => { navigation.navigate('ViewOrder') }
        },
        
    ]

    const optionCards = (title, icon, action) => {
        return (
            <TouchableOpacity
                style={styles.options}
                onPress={() => action()}
                key={title}
            >
                <Icon name={icon} color={constantColors.default} size={25} />
                <Text>       </Text>
                <Text style={{ flex: 1, fontSize: 16, color: constantColors.default }}>
                    {title}
                </Text>
            </TouchableOpacity>
        )
    }

    const ImageView = () => {
        if(user.image == null) {
            return (
                <Image source={{uri: `data:image/png;base64,${userProfilePicture}`}} style={{ width: 100, height: 100, borderRadius: 100 / 2, marginRight: 20 }} />
            )
        } else {
            return (
                <Image source={{uri: `data:image/jpeg;base64,${user.image}`}} style={{ width: 100, height: 100, borderRadius: 100 / 2, marginRight: 20 }} />
            )
        }
    }

    return (
        <View style={styles.container}>
            <View style={styles.navigations} >
                <ImageView />
                <View style={{

                }}>
                    <Text style={{ fontSize: 20, fontWeight: 'bold', color: constantColors.default }}>{user.name}</Text>
                    {/* <Text style={{ fontSize: 16, fontWeight: 'bold', color: constantColors.default }}></Text> */}
                    <Text style={{ fontSize: 16, fontWeight: '600' }}>{user.address}</Text>
                    <Text style={{ fontSize: 16, fontWeight: '600' }}>{user.phone_number}</Text>
                    
                </View>
            </View>
            <View style={styles.body}>
                <ScrollView>
                    {options.map(({ title, action, icon }) => {
                        return optionCards(title, icon, action);
                    })}
                    </ScrollView>
            </View>
            <View style={styles.footer}>
                <TouchableOpacity  
                    style={styles.optionsLogout}
                    onPress={() => signOut()}

                >
                    <Icon name='exit-to-app' color={constantColors.default} size={25} />
                    <Text>       </Text>
                    <Text style={{ flex: 1, fontSize: 16, color: constantColors.default }}>
                        Logout
                    </Text>
                </TouchableOpacity>
            </View>
        </View>
    )
}

export default Profile

const styles = StyleSheet.create({
    container: {
        flex: 3,
    },
    navigations: {
        flex: 3,
        marginTop: 5,
        padding: 10,
        flexDirection: 'row',
        justifyContent: 'flex-start',
        alignItems: 'center'

    },
    searchBar: {
        display: 'flex',
        height: 40,
        //justifyContent: 'space-around',
        flexDirection: 'row',
        marginLeft: 4,
    },
    body: {
        flex: 10,
        display: 'flex',
    },
    emptyCart: {
        flex: 1,
        display: 'flex',
        justifyContent: 'center',
        alignItems: 'center'
    },
    emptyCartText: {
        fontSize: 25,
        fontWeight: '700'
    },
    imgCartEmpty: {
        width: 250,
        height: 150,
    },
    footer: {
        flex: 2,
        padding: 10,
       
    },
    ammountContainer: {
        display: 'flex',
        flexDirection: 'row',
        justifyContent: 'space-between',
        paddingLeft: 20,
        paddingRight: 20

    },
    popUpContainer: {
        display: 'flex',
        flexDirection: 'column',
        justifyContent: 'space-around',
        width: '100%',
    },
    paymentView: {
        flexDirection: 'row',
        justifyContent: 'space-around',
        padding: 8,
        margin: 4,
        backgroundColor: constantColors.default,

    },
    paymentOption: {
        display: 'flex',
        flex: 1,
        flexDirection: 'row',
        justifyContent: 'center',
        alignItems: 'center',
        paddingLeft: 20
    },
    options: {
        backgroundColor: '#F2F2F2',
        height: 70,
        justifyContent: 'space-around',
        flexDirection: 'row',
        alignItems: 'center',
        padding: 10,
        borderBottomWidth: 3,
        borderTopWidth: 3,
        borderColor: '#fff'
    },
    optionsLogout: {
        backgroundColor: '#F2F2F2',
        height: 70,
        justifyContent: 'space-around',
        flexDirection: 'row',
        alignItems: 'center',
        borderBottomWidth: 3,
        padding: 10,
        borderTopWidth: 3,
        borderColor: '#fff'
    },
})
